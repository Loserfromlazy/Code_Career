...
--认证redis密码 如果l需要加上这段redis的认证脚本
ok,err = red:auth(config.password)
if not ok then
    ngx.say("failed to auth:",err)
    return
else
    ngx.say("success to auth","<br>")
end
...
--取值
local res ,err = red:get("blue")
if not res or res == ngx.null then
    ngx.say("failed to get value",err,"<br>")
    return
else
...
for i, v in ipairs(results) do
    if type(v) == "table" then
        if v[1] == false then
            ngx.say("failed to run command",i,":",v[2],"<br>")
        else
            ngx.say("success to run command",i,":",v[i],"<br>")
        end
    else
        ngx.say("success to run command",i,":",v[i],"<br>")
    end
