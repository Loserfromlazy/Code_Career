...
![image-20220603224902511](C:\Users\Yuhaoran\AppData\Roaming\Typora\typora-user-images\image-20220603224902511.png)

虽然在内存中User对象是以字节的形式存储的，但是其存储在内存中的属性值，并不是具体的值，而是其值的内存地址，如果服务端把这些内存数据直接传输到客户端，客户端凭借这些内存地址是肯定找不到张三的值，也找不到18的值。这时候就需要进行序列化，把Java对象转换成字节序列，用来进行远程传输或保存到硬盘上。但是这些字节需要同时还需要恢复成Java对象
