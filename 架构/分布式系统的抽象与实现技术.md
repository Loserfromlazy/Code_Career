...
容错的目的就是为了实现高可用。也就是说对外提供服务时，即使出现了一些硬件故障，还是可以稳定的对外提供服务。实现这种容错的方法或者说是工具就是使用复制。但是复制也不是万能的，那么复制能解决什么样的问题呢？

答案就是是fail-stop故障。解释一下Fail-stop是一种容错领域的通用术语。如果某些东西出了故障，那么就会停止运行，而不是运算出错误结果。=。比如计算机，假如服务器的电源线或网线被拔掉了，那就会产生一个fail-stop故障。因为从外界来看，服务器和停止运行没有两样。

然而，复制无法解决软件中的bug和硬件设计中的缺陷问题。以第一个实验MapReduce为例，即使将master复制两份并在服务器上运行，两份计算仍会产生相同错误的结果。

同时通过复制来实现高可用还有一个问题需要考虑，假如数据中心所在的城市发生了地震，摧毁了整个数据中心，无论我们在那个数据中心里有多少副本，都无济于事。因为这种由地震，停电，建筑失火引起的问题，如果多个副本在同一个建筑中，那么这类问题是副本之间关联的错误。所以，如果我们想处理类似地震引起的问题，我们需要将我们的副本放在不同的城市，或者至少物理上把它们分开，这样它们会有独立的供电，不会被同样的自然灾害影响。也就是实现异地的灾备

> 通过异地灾备实现跨地域高可用，提升数据安全性和系统可用性。当发生机房或数据中心级别故障时，可以快速恢复业务。可以实现两地三中心、两地四中心、三地六中心等架构。
>
> 1. **两地三中心（2-3架构）**：
>    - 这意味着有两个不同的地理位置，总共部署了三个数据中心。
>    - 通常，这种架构包括一个主数据中心和两个灾备中心，或者两个主数据中心和一个灾备中心。
>    - 这种架构的目的是在主数据中心不可用时，可以启用备份中心，同时确保数据的备份和恢复能力。
> 2. **两地四中心（2-4架构）**：
>    - 这表示有两个地理位置，每个位置部署了两个数据中心，总共四个。
>    - 这种架构通常用于实现更高的数据冗余和可靠性。例如，每个地点都可能有一个主数据中心和一个备份中心。
>    - 这样可以确保即使一个完整的地点发生故障，另一个地点仍可继续运行。
> 3. **三地六中心（3-6架构）**：
>    - 指的是分布在三个不同地理位置的六个数据中心。
>    - 这种架构提供了最高级别的冗余和灾难恢复能力，适用于那些对数据安全和业务连续性要求极高的场景。
>    - 每个地点可能有两个数据中心，其中一个作为主中心，另一个作为备用中心，或者实施更复杂的多活策略。

那么以上讨论的复制方案是否值得我们实际使用呢？因为这涉及到需要的2-3倍的计算机资源。实际上，这个问题取决于经济价值，与技术无关。如果是正在运营的一个银行系统，并且发生计算机宕机意味着无法为用户提供服务，这将导致收入减少并使用户不满意，那么花费1000-2000美元购买一台备用计算机可能是值得的。在这种情况下，可以有一个额外的副本来保证连续性。然而，如果涉及到一个课程网站，那么拥有一个热备份并不值得，因为课程网站宕机的后果相对较小。因此，系统复制是否值得以及需要复制多少份，取决于服务宕机会带来多大的损失和不便。

### 5.1.2 复制的方法

在VMware FT论文中介绍了两种复制的方法，一种是状态转移（State Transfer），另一种是复制状态机（Replicated State Machine）。

如果我们有一个服务器的两个副本，它们需要同步以互相备份。这样，当主服务器（Primary）出现故障时，备份服务器（Backup）拥有所有必要信息以接管服务。主服务器会定期将其状态，比如内存中的内容，复制并发送到备份服务器。备份服务器保留收到的最新状态，所以当主服务器故障时，它能从最新状态开始运行。状态转移就是这样的一个过程：发送主服务器的状态至备份服务器。主服务器会定期复制自己的内存，并通过网络发送给备份服务器。为了提高效率，只有在最后一次同步后发生变化的内存部分会被发送。

复制状态机（Replicated State Machine）的原理是：大多数服务或计算机软件都执行一些确定的内部操作，只有外部输入是不确定的。如果一台计算机不受外部影响，那么它只会顺序执行指令，这些指令基于内存和寄存器中的函数。只有当外部事件介入时，如接收到网络数据包，才会发生变化。因此，复制状态机不会在副本间发送状态，而是将这些外部事件（如客户端输入）从主服务器发送到备份服务器。如果两台计算机从相同状态开始，并且以相同的顺序和时间接收相同的输入，那么它们会保持同步并一致。

总结一下，状态转移涉及状态（比如内存）的传输，而复制状态机则是将来自客户端的操作或其他外部事件从主服务器传输到备份服务器。

通常来说，外部操作或者事件比服务的状态要小。比如是一个数据库的话，它的状态可能是整个数据库，可能到达GB这个级别，而外部操作只是一些客户端发起的请求，例如读数据。所以操作通常来说比较小，而状态通常比较大。但是复制状态机的缺点是，它会更复杂一些，并且对于计算机的运行做了更多的假设。而状态转移就比较简单粗暴，就是将我整个状态发送给你，你不需要再考虑别的东西。

> 这里有学生提问：随机操作在复制状态机会怎么处理？
>
> Robert教授：这是个好问题。当没有外部的事件时，Primary和Backup都执行相同的指令，得到相同的结果，复制状态机才有意义。对于ADD这样的指令来说，这是正确的。如果寄存器和内存都是相同的，那么两个副本执行一条ADD指令，这条指令有相同的输入，也必然会有相同的输出。但是，如你指出的一样，有一些指令，或许是获取当前的时间，因为执行时间的略微不同，会产生不同的结果。又或者是获取当前CPU的唯一ID和序列号，也会产生不同的结果。对于这一类问题的统一答案是，Primary会执行这些指令，并将结果发送给Backup。Backup不会执行这些指令，而是在应该执行指令的地方，等着Primary告诉它，正确的答案是什么，并将监听到的答案返回给软件。

下面我们来详细讨论一下复制状态机这种方法。（这也是论文中主要介绍的方式）

如果我们想构建一种复制状态机的解决方案，我们需要考虑以下几个问题：

1. 如何决定主副本的同步频率

   Primary往往比Backup更快执行指令，因为Primary接收外部输入，而Backup需要稍作滞后。这也意味着如果Primary故障，Backup可能未完全同步。然而，完全同步会导致高延迟，因为需要大量交互。因此，在设计中通常会关注同步频率的平衡。

2. 如何实现主副本切换

   如果主服务器发生故障，系统必须切换到备用服务器，并通知所有客户端与新的主服务器通信。如果想设计一个完全无异常的切换系统几乎不可能。理想情况下，当主服务器出现问题时，系统会无缝切换到备用服务器，而客户端毫无察觉。但这在实际中很难实现，因此在切换过程中的异常是不可避免的，我们需要找到处理这些异常的方法。

3. 如何实现副本的创建

   当我们的某个副本出现故障时，需要迅速添加新副本以保持系统稳定。特别是在只有两个副本的情况下，一旦一个出现故障，整个服务就面临风险，因为另一个副本也可能随时故障。尽管添加新副本成本高昂（因为副本状态往往非常庞大，但这是必要的）。如果要创建一个新的副本，只能使用状态转移，因为新的副本需要有完整状态的拷贝。所以创建一个新的副本，代价会很高。

4. 什么样的状态需要被复制

   VMware FT论文对这个问题有一个非常有趣的回答。它会复制机器的完整状态，这包括了所有的内存，所有的寄存器。这是一个非常非常详细的复制方案。

   对于复制方案来说，这种类型是非常少见的。总的来说，大部分复制方案都跟GFS更像。GFS也有复制，但是它绝对没有在Primary和Backup之间复制内存中的每一个bit，它复制的更多是应用程序级别的Chunk。应用程序将数据抽象成Chunk和Chunk ID，GFS只是复制了这些，而没有复制任何其他的东西，所以也不会有复制其他东西的代价。对于应用程序来说，只要Chunk的副本的数据是一致的就可以了。基本上除了VMware FT和一些屈指可数的类似的系统，其他所有的复制方案都是采用的类似GFS的方案。也就是说基本上所有的方案使用的都是应用程序级别的状态复制，因为这更加高效。

   总的来说，大部分场景都是应用程序级别的复制。

VMware FT的独特之处在于它能够在机器级别进行复制，无论你在机器上运行何种软件，它只关注底层的寄存器和内存的复制。你可以在VMware FT管理的机器上运行任何软件，只要该软件适用于VMware FT支持的微处理器。这种方法的缺点是效率不高，但优点是你可以在VMware FT的复制方案中运行任何现有的软件，甚至无需拥有该软件的源代码或理解其内部工作方式。在一定的限制条件下，你可以将这些软件应用于VMware FT的复制环境中。

## 5.2 VMware FT

首先，VMware是一个虚拟机公司，通常一台计算机只能在硬件上启动一个操作系统。但是如果在硬件上运行一个虚拟机监控器（VMM，Virtual Machine Monitor）或者Hypervisor，Hypervisor会在同一个硬件上模拟出多个虚拟的计算机。所以通过VMM，可以在一个硬件上启动一到多个Linux虚机，一到多个Windows虚机。如下图：

![image-20240105132650640](https://mypic-12138.oss-cn-beijing.aliyuncs.com/blog/picgo/image-20240105132650640.png)

虚拟机技术是VMware的发家技术，并且它有大量围绕这个技术构建的复杂系统。

### 5.2.1 工作原理

VMware FT需要两个物理服务器。Primary在其中一个物理服务器上，Backup在另一个物理服务器上，它与Primary有着一样的操作系统。注意将Primary和Backup运行在一台服务器的两个虚拟机里面毫无意义，因为容错本来就是为了能够抵御硬件故障。所以，至少需要两个物理服务器。

在两个物理服务器上，每个虚拟机管理器（VMM）为各自的虚拟机分配内存，并确保这两个虚拟机的内存镜像完全一致。所以到现在为止我们有两个物理服务器，每个上运行一个虚拟机，这些虚拟机中都包含了我们所需服务的副本。

让这两台服务器通过网络连接。除此之外，在这个局域网（LAN，Local Area Network），还有一些其他客户端。这里客户端不止代表需要与服务器交互的客户端，同时还有一些其他需要交互的，比如Disk Server(远程磁盘)，这里可以将远程盘服务器也看做是一个外部收发数据包的源，与客户端的区别不大。VM FT的整体架构如下（图片来自论文）：

![image-20240105132956782](https://mypic-12138.oss-cn-beijing.aliyuncs.com/blog/picgo/image-20240105132956782.png)

上图根据课堂内容细化后如下：

![image-20240105134148370](https://mypic-12138.oss-cn-beijing.aliyuncs.com/blog/picgo/image-20240105134148370.png)

基本的工作流程如下，先看大体的流程图：

![image-20240105134947090](https://mypic-12138.oss-cn-beijing.aliyuncs.com/blog/picgo/image-20240105134947090.png)

现在有某些服务的客户端，向Primary发送了一个请求（见上图序号1），这个请求以网络数据包的形式发出。这个网络数据包产生一个中断，之后这个中断送到了VMM。VMM发现这是一个发给服务的一个输入，所以这里VMM会做两件事情：

* 在虚拟机的操作系统中，模拟网络数据包到达的中断（见上图序号3），并将相应的数据发送给在Primary跑着的应用程序（见上图序号4）。
* 除此之外，因为这是一个多副本虚拟机的输入，VMM会将网络数据包拷贝一份，并通过网络送给Backup虚拟机所在的VMM（见上图序号2）。

Backup虚拟机的VMM知道这是发送给Backup网络数据包，它也会在Backup虚拟机中模拟网络数据包到达的中断，并将数据发送给Backup跑着的应用程序。

虚拟机的服务回复客户端的请求，所以在Primary虚拟机中，服务会生成回复报文（见上图序号5），并通过VMM的虚拟网卡发出（见上图序号6），它会实际的将这个报文发送给客户端。另一边，在Backup中也运行了相同顺序的指令，它也会生成一个回复报文给客户端，并将这个报文通过它的VMM模拟出来的虚拟网卡发出。但是VMM知道这是Backup虚拟机，所以会丢弃这里的回复报文（见上图序号8）。所以只有Primary虚拟机实际生成了回复报文给客户端（见上图序号7）。

这里VMware FT论文中将Primary到Backup之间同步的数据流的通道称之为Log Channel。从Primary发往Backup的事件被称为Log Channel上的Log Event/Entry。（如下图，图片来自论文）

![image-20240105114712411](https://mypic-12138.oss-cn-beijing.aliyuncs.com/blog/picgo/image-20240105114712411.png)

当Primary因为故障停止运行时，FT（Fault-Tolerance）就开始工作了，这时Backup会上线（Go Alive）。这意味着，Backup不会再等待来自于Primary的Log Channel的事件，Backup的VMM会让Backup自由执行，而不是受来自于Primary的事件驱动。Backup的VMM会在网络中做一些处理让后续的客户端请求发往Backup，而不是Primary。现在这个Backup成为了Primary，它直接接收输入，直接产生输出。到此为止，Backup接管了服务。

> 这里VM FT是通过UDP心跳和log channel的日志流量和ack确认机制，来进行失败检测的。因为由于定时器中断，日志流量应该是规律的，且对于正常工作的客户操作系统来说不应该停止。因此，日志条目或确认信息的流动停止可能表明虚拟机出现了故障。如果心跳或日志流量停止的时间超过了特定的超时时限（大约几秒钟的数量级），将宣布发生故障。
>
> 类似的如果Backup虚机停止运行，Primary也需要用一个类似的流程来抛弃Backup，停止向它发送事件。
>
> 更加详细的描述可见论文2.3章节（FT系统如何检测和响应VM故障）和3.1章节（FT VM的启动和重启机制，包括如何在主VM故障后恢复备份VM）。

### 5.2.2 非确定性事件的处理

上一节简单介绍了Backup也看到了来自客户端的请求，经过同样的执行过程，并与Primary保持一致，但是这背后有很多很重要的细节。其中一个问题是存在非确定性（Non-Deterministic）的事件。尽管代码执行在多数情况下是根据编写的指令直接进行的，但计算机执行的行为并不完全只受其内存中的指令内容影响。换言之，除了代码指令以外，还有其他因素可能影响计算机指令的执行。

非确定性事件可以分成几类：

- 客户端输入：

  客户端的输入是不可预期，客户端请求何时送达，会有什么样的内容，并不取决于服务当前的状态。在VM FT中输入是通过网络数据包形式接收的，包含两个部分：数据包内容和数据包到达时触发的中断。为了保持主备状态的一致性，中断需要在主备系统中以相同的时间和位置触发。这是因为中断的处理时机和位置如果不一致，将导致主备系统执行相同指令时的状态发生偏差。因此，除了关心网络数据包的内容，还需确保中断的时间处理得当，以维护系统的一致性。

- 特殊指令：

  有一些指令在不同的计算机上的行为是不一样的，比如随机数生成器、获取当前时间的指令，在不同时间调用会得到不同的结果、获取计算机的唯一ID等。

- 多核CPU并发：

  在VMware FT论文中没有讨论多核CPU的并发，之所以多核会导致非确定性事件，是因为当服务运行在多CPU上时，指令在不同的CPU上会交织在一起运行，进而产生的指令顺序是不可预期的。
...
>
> 其中这里课程的要求是LEC5需要读完前5章。LEC6需要从第7章读到结尾。
>
