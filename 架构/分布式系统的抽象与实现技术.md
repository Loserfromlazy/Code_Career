...
- filename ->array of chunk handles
  ...
  文件名到chunk ID或者chunk handle的映射。即文件对应了哪些chunk服务器。
...
> 存储在内存的原因是为了读取快且方便周期性扫描元数据。虽然chunk的数量以及整个系统的容量受到主服务器拥有多少内存的限制，但是GFS也使用了前缀压缩算法进行了压缩。GFS这么做是因为加内存的成本是很低的。（详见论文原文2.6.1章）

讲的更具体的话就是Master会在磁盘上存储log，每次有数据变更时，Master会在磁盘的log中追加一条记录，并生成CheckPoint（检查点）。这里使用log的原因是，数据库本质上来说是某种B树或者hash table，追加写入log是很高效的。因为
