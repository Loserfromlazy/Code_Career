...
在秒杀等场景下会有很高的流量，如果没有相应的保护，将导致系统超负荷甚至崩溃，但是如果进行过多的限流，那么就会极大的降低用户的体验，因此此时就可以利用RocketMQ来解决。

一般来说秒杀场景下的高并发问题的主要瓶颈都在数据库上，因此针对秒杀可以进行如下优化：

1. 前端设置验证码，防止刷单
2. 秒杀系统作为独立服务部署，防止对其他服务比如订单商品等模块造成影响
3. 库存先写入Redis，基于Redis做高并发下的库存预扣减，一旦库存不足，就拒绝后续的秒杀
4. 扣减库存成功的秒杀请求，先进入MQ，然后由其他服务比如订单订阅消息并下单。

总体来说是使用高低速服务，即高速的接入层通过mq进行缓冲，再由低速的服务层逐步进行处理，如下图：

![image-20230906230839330](https://mypic-12138.oss-cn-beijing.aliyuncs.com/blog/picgo/image-20230906230839330.png)

### 1.3.2 异步解耦

微服务框架下，服务与服务之间有很多的交互，一种方式是通过RPC进行调用，另一种就是通过RocketMQ进行异步的交互。如下图：

![image-20230906230823682](https://mypic-12138.oss-cn-beijing.aliyuncs.com/blog/picgo/image-20230906230823682.png)

这样秒杀服务只需要与RocketMQ消息队列对接即可，这样实现上下游业务系统的松耦合设计，而不是每一次修改都需要针对各个服务进行修改和重新开发。

### 1.3.3 顺序消息

与FIFO类似，RocketMQ提供顺序消息，可以保证消息的先进先出，可以用于交易系统的订单创建、支付、退款等有顺序的业务上。

### 1.3.4 事务消息

RocketMQ还提供了事务消息，使用事务消息可以保证数据的最终一致性。比如有以下一个业务，当用户支付完订单后，可以得到100积分，而微服务下订单和积分是两个独立的模块，这时就会产生事务问题，我们需要保证支付和积分是原子的操作。这时这种带事务的业务就可以用RocketMQ来解决，如下图：

![image-20230906230805789](https://mypic-12138.oss-cn-beijing.aliyuncs.com/blog/picgo/image-20230906230805789.png)
...
- ConsumeQueue是消息消费队列，当消息到达CommitLog文件后，将异步转发到ConsumeQueue文件中，供消息消费者进行消费。
...
由于namesrv节点是无状态的，且各个节点的数据是一致的，所以存在多节点namesrv的情况下，部分namesrv不可用也可以保证MQ服务正常运行。下面我们来做个实验：
...
- 由于Slave只负责读，所以该Master对应的Slave仍能保证消息被正常消费
...
生产者高可用实现非常简单，比如我们有一个消息服务作为生产者提供API接口，其他服务通过调用消息服务的接口来发送消息，这时如果想保证消息服务即生产者的高可用，只需要部署多个服务即可。
...
- 集群消费模式（默认）：当使用集群消费模式时，RocketMQ 认为任意一条消息只需要被消费组内的任意一个消费者处理即可。
- 广播消费模式：当使用广播消费模式时，RocketMQ 会将每条消息推送给消费组所有的消费者，保证消息至少被每个消费者消费一次。
...
如果一个Broker组有master和slave，消息需要从master复制到Slave上，有同步和异步两种复制方式，区别是在同步模式下只有主从复制完毕才会返回给客户端；而在异步模式下，主从的复制时异步的，不用等待即可返回。

同步模式下，只有当Master和Slave均写入成功后才反馈给客户端写成功状态。具体流程是当消息写入内存的Page Cache后，立即通知刷盘线程刷盘，然后等待刷盘完成，刷盘线程执行完成后唤醒等待的线程，给应用返回消息写成功的状态。

这样做的好处是可以保持消息主从的状态一致，如果Master出故障，Slave上有全部的备份数据，消息不丢失。缺点就是性能很低，同步模式会增加数据写入延迟，降低系统吞吐量，性能比异步复制模式低（大约10%）。

异步模式，只要Master写成功即可反馈给客户端写成功状态。这样的优点是系统有较低的延迟和较高的吞吐量。Master宕机之后，消费者仍可以从Slave消费，但是有些数据会因为没有被写入Slave，而丢失少量消息。

二者的对比如下：

|        | 同步复制                                             | 异步复制                                                     |
| ------ | ---------------------------------------------------- | ------------------------------------------------------------ |
| 概念   | 等Master和Slave均写入成功后才反馈写成功状态          | 只要Master写成功，就反馈客户端写成功                         |
| 可靠性 | 可靠性高，Slave上有全部的备份数据                    | 如果Master出现故障，可能存在一些数据没来得及写入Slave，可能会有消息丢失 |
| 效率   | 性能很低，同步模式会增加数据写入延迟，降低系统吞吐量 | 有较低的延迟和较高的吞吐量                                   |

**Broker消息零丢失的权衡**

在消息的零丢失是一把双刃剑，需要结合具体的业务场景，在性能和消息零丢失上做平衡。在实际应用中一般会将Master和Slave设置成异步刷盘方式，主从之间设置成同步复制方式。

这样的话即使有一台机器出故障仍然可以保证数据不丢失。且异步刷盘能避免频烦触发磁盘写操作，除非服务器宕机，否则不会造成消息丢失。

### 5.2.2 刷盘：同步和异步刷盘

- 异步刷盘

  在返回应用写成功状态前，消息只是写入pagecache，并没有落盘。这样能充分利用OS的pagecache的优势，只要消息写入pagecache即可将成功的ACK返回给Producer。消息刷盘由后台异步线程的方式进行，降低了读写延迟，提高了性能和吞吐量。

- 同步刷盘

  在返回应用写成功状态前，消息已经被写入磁盘。只有在消息真正持久化至磁盘后RocketMQ的Broker端才会真正返回给Producer端一个成功的ACK响应。
  RocketMO的消息是存诸到磁盘上的，这样既能保证断电后恢复，又可以让存储的消息量超出内存的限制。RocketMO为了提高性能，会尽可能地保证磁盘的顺序写。
  同步刷盘对MQ消息可靠性来说是一种不错的保障，但是性能上会有较大影响，一般适用于金融业务应用该模式较多。

同步复制和异步复制是通过Broker配置文件中的brokerRole参数进行设置的，这个参数可以被设置为ASYNC_MASTER（异步）、SYNC_MASTER（同步）、SLAVE（从节点）

## 5.3 消费者消息的可靠消费

### 5.3.1 合理使用重试策略

消费者消费消息后，需要给Broker返回消费状态，以`MessageListenerConcurrently`监听器为例，Consumer消费完成后必须返回消费状态，这个是一个枚举，源码如下：
...
1. 异常重试
   ...
   因为Consumer逻辑出现异常，导致返回`RECONSUME_LATER`状态，那么Broker就会在一段时间后重试

2. 超时重试

   若是Consumer端处理时间过长，或者因为某些线程挂起，导致迟迟没有返回消费状态，Broker就会认为Consumer消费超时，此时会发起重试。若是正常完成消费，则直接返回消费成功。

### 5.3.2 消费者业务异常

如果消息处理发生异常，这时我们可以返回`RECONSUME_LATER`表示稍后重试，例如下面的代码：
...
当返回`RECONSUME_LATER`消息就会进入到重试队列。

### 5.3.3 重试队列

当一条消息首次消费失败，消息队列会自动进行重试：

- 代码正常执行返回为CONSUME_SUCCESS
- 执行异常返回RECONSUME_LATER

我们可以在控制台查看重试队列：

![image-20230918100247477](https://mypic-12138.oss-cn-beijing.aliyuncs.com/blog/picgo/image-20230918100247477.png)

消费者所属的消息组为`GROUPA`，那么其重试队列的名称就是`%RETRY%GROUPA`,重试队列的消息过一段时间就会再次发送给消费者，如果还是无法正常执行，就会再次进入重试队列，默认一共会重试16次，如果还是无法执行就会进入到死信队列。

- 对于无序消息（普通、延时、事务消息），当consumer消费失败时，可以通过设置返回状态实现消息重试
- 无序消息只支持集群消费模式，不支持广播消费模式，广播模式不支持失败重试特性
- 对于顺序消息，为了保证消息消费的顺序性，当consumer消费失败后，消息队列会自动不断的进行消息重试，这会导致consumer消费被阻塞的情况，所以必须保证应用能及时监控并处理消费失败的情况，避免阻塞现象的发生。

消息默认最多被重试16次，每次的间隔如下：

| 第几次重试 | 间隔时间 | 第几次重试 | 时间间隔 |
| ---------- | -------- | ---------- | -------- |
| 1          | 10s      | 9          | 7min     |
| 2          | 30s      | 10         | 8min     |
| 3          | 1min     | 11         | 9min     |
...
一个消息无论重试多少次，其MessageID都不会改变。

当然重试次数也可以自定义，最大重试次数小于等于16次，间隔同上表。大于16次，超过16的间隔都是2h。设置方式如下：

```java
consumer.setMaxReconsumeTimes(20);
```

注意：最大重试次数对相同Group ID下的所有consumer有校。如果对相同Group ID的两个consumer其中一个进行设置，那么两个都生效。且配置采用覆盖生效，即最后启动的consumer会覆盖之前的consumer的配置。

### 5.3.4 死信队列

死信队列的名字是`%DLQ%GROUPA`。也可以开一个线程订阅死信队列，并不断重试。可以在控制台查看：

![image-20230918104030421](https://mypic-12138.oss-cn-beijing.aliyuncs.com/blog/picgo/image-20230918104030421.png)

死信队列不会再被消费者正常消费，且有效期与正常消息相同，均为3天，3天后会被自动删除。一个死信队列对应一个消费者组，而不是单个consumer。且一个死信队列包含了对应的Group ID的所有死信消息，不论消息属于哪个Topic。如果一个Group ID从未有死信消息，则不会创建死信队列。

一旦一条消息进入了死信队列，意味着某些原因导致消费者无法正常消费，因此通常需要进行特殊处理，排查其他因素后，可以在控制台重新发送消息，重新消费一次。

### 5.3.5 消息投递的重复问题

先来了解一下QoS（Quality of Service 服务质量），一般分为：

- At Most once

  最多一次，即发送一次无论成败都不会再重发。

- At Last once

  最少一次，即消息投递后，如果未收到ack，则再次投递。每个消息都必须投递一次Consumer先拉取消息到本地，消费完成后，向服务端返回ack，如果没有消费则一定不回复ack，所以RocketMQ支持此特性。

- Exactly Only once

  发送消息时，不允许发送重复的消息。消费消息时，不允许消费重复的消息。只有这两个条件全满足，才可以算是Exactly Only once。

一般如果想实现Exactly Only once在分布式环境下需要大量额外开销，所以RocketMQ并不保证此特性，RocketMQ要求在业务上进行去重，也就是保证消息的幂等性。

RocketMQ虽然不保证不重复，但是正常很少出现重复发送、消费的情况。只有网络异常，consumer的启动停止等异常情况下才会出现消息重复。此问题的本质原因是网络调用存在不确定性，比如: 网络原因闪断，ACK返回失败等等故障，确认信息没有传送到消息队列，导致消息队列不知道自己已经消费过该消息了，再次将该消息分发给其他的消费者，所以才产生了消息重复性问题。

> 不同的消息队列发送的确认信息形式不同,例如RabbitMo是发送一个ACK确认消息，RocketMO是返回一个CONSUME SUCCESS成功标志，kafka实际上有个offset的概念。

### 5.3.6 消息的幂等

RocketMQ并不保证一条消息只被推送一次，因此一条消息可能会被消费多次，所以消费者在接收到消息后，有必要对消息做幂等处理。

幂等性就是指一次请求和多次请求同一个资源产生相同的副作用。对于一个消息来说就是一次消费和多次消费产生的副作用是一样的。一般如果相同的请求调用一个接口一次或多次，需要给调用方提供一致的结果时，就需要考虑设计成幂等接口，即`f(x)=f(f(x))`。

在系统高并发的环境下，很有可能因为网络，阻塞等等问题导致客户端或者调用方并不能及时的收到服务端的反馈甚至是调用超时的问题。比如订单系统可能会遇到以下问题：

1. 创建订单时，第一次调用服务超时，再次调用是否会产生两笔订单
2. 扣减库存时第一次调用超时，再次调用是否会多扣减一次。

如果保证了幂等性，上述问题就不会产生两笔订单，也不会多扣减一次库存。

一般在分布式、微服务架构中可能发生重复请求或消费的场景，比如：

- 网络波动
- 分布式消息消费
- 用户重复消费
- 未关闭的重试机制（比如Nginx重试、Rpc通信重试等）

CURD幂等性分析：

- 新增类：不具备幂等性
- 查询类：重复查询不会产生或更新数据，所以具有天然幂等性
- 更新类：
  - 基于主键的计算式的update不具备幂等性：update goods set numer = numer -1
  - 基于主键的非计算式的update具备幂等性：update goods set numer = newNumber
  - 基于条件查询的更新，需要具体问题具体分析
- 删除类：
  - 基于主键的删除有幂等性
  - 基于主键的逻辑删除（update）也具有幂等性

针对一个微服务架构如果不支持幂等性则会产生以下问题：
...
- 全局唯一ID
  ...
  如果使用全局唯一ID，就是根据业务的操作和内容生成一个全局ID，在执行前先根据这个全局唯一ID是否存在来判断这个操作是否已经执行。如果不存在全局ID，则存储到数据库、Redis中，如果存在则代表消息已经执行。使用全局ID是一个通用方案。

- 数据库唯一索引

  这个方法适用于在业务中有唯一标识的插入场景，比如在支付场景中，如果一个订单只支付一次，所以订单ID可以作为唯一标识，这时可以建一张去重表，并且把唯一标识作为唯一索引。我们可以将创建支付单据写入去重表，放进一个事务中，如果重复创建，数据据会抛出异常，事务会回滚。

- 插入或更新

  这种方法适用于插入且有唯一索引的情况，比如关联商品品类，其中商品的ID和品类可以构成唯一索引，并且在数据表也增加了唯一索引，这时就可以使用Insert或Update操作。

- 状态机

  这种方式适用于有状态流转的情况，比如订单的创建和付款，订单的付款肯定是前，这时可以通过状态字段控制，我们可以设计状态字段并通过比较做幂等，比如：`update table set status = #{status} where id =#{id} and status<=#{status}`

- 多版本控制

  这种方法适合在更新的场景中，比如我们要更新商品的名字，这时我们就可以在更新的接口中增加一个版本号，来做幂等，比如`update table set name=#{name} ,version = #{version} where id =#{id} and version < #{version}`

- 其他方案...

以上就是保证幂等性的常见方法，但是幂等性设计不能脱离业务进行讨论。
...
- 发送时消息重复
  ...
  当一条消息已被成功发送到服务端并完成持久化，此时出现了网络波动或客户端宕机，导致服务端对客户端应答失败。此时如果生产者意识到消息发送失败并尝试再次发送消息，消费者后续会收到两条内容相同且Message ID相同的消息。

- 投递时消息重复

  消息消费的场景下，消息已投递到消费者并完成业务处理，当客户端给服务端反馈应答的时候网终闪断。为了保证消息至少被消费一次，消息队列 RocketMO 的服务端将在网络恢复后再次尝试投递之前已被处理过的消息，消费者后续会收到两条内容相同并且 Message ID 也相同的消息。

- 负载均衡时消息重复

  当RocketMO 的 Broker 或客户端重启、扩容或缩容时，会触发 Rebalance，此时消费者可能会收到重复消息。
