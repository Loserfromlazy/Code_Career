...
> 除了划分空间，分配内存还要考虑创建对象是否频繁，在并发情况下能否线程安全。解决此问题有两种方案：一是对分配内存空间的动作进行同步处理（通过CAS加失败重试保证更新操作的原子性），另一种就是把内存分配的动作按照线程划分在不同空间中，每个线程在Java堆中预先分配一小块内存称为本地线程分配缓冲（TLAB），然后哪个线程要分配就在对应的本地缓冲中分配，只有本地缓冲区用完了需要分配新的缓冲区时才需要同步锁定。虚拟机是否使用TLAB可通过`-XX: +/-UseTLAB`参数设定。
...
虽然这些地方都有可能设置安全点，但是HotSpot虚拟机为了避免安全点过多带来过多过重的负担，对循环还有一项优化措施，认为循环次数较少的话，执行时间也不会太长，所以使用int类型或者范围更小的数据类型作为索引值的循环默认是不会被放置安全点的，这种循环被称为可数循环（Counted Loop）,相对应的使用long或更大范围的数据类型作为索引值的循环被称为不可数循环（Uncounted Loop），将会被防止安全点，通常情况下这个优化是可行的，但是循环执行的时间不单单是由次数决定，如果循环体单次执行特别慢，即便是可数循环也可能会消耗很多的时间。
...
下面我们来推导（推导过程与原书一致，仅图片不同，图片是我自己按照原书步骤画的）把原本存活的对象标为死亡这种致命错误出现的原因：
...
**选择一款合适的垃圾收集器主要受一下三点影响 

- 权衡程序的主要关注点，如果是数据分析、科学计算的任务，目的是尽快算出结果，那吞吐量就是主要关注点；如果是SLA应用，那么停顿时长会直接影响服务质量，严重的会导致事务超时，如果是客户端和嵌入式应用，那么GC占用内存是不可忽视的。
- 其次要考虑运行的基础设施，比如硬件规格，系统架构，处理器的数量，分配内存的大小，操作系统是Linux、Solaris或者Windows等
- 还要考虑JDK的发行商，对应的版号，是OracleJDK、OpenJDK等，该JDK对应了哪个版本？

一般来说，GC主要是从上面的几个方向考量。从理论上分析如果你有充足的预算但没有太多调优经验，那么一套带商业技术支持的专有硬件或者软件解决方案是不错的选择，Azul公司以前主推的Vega系统和现在主推的Zing VM是这方面的代表，这样你就可以用传说中的C4收集器了；如果你虽然没有足够预算去使用商业解决方案，但能够掌控软硬件型号，使用较新的版本，同时又特别注重延迟，那ZGC很值得尝试；如果你对还处于实验状态的收集器的稳定性有所顾虑，或者应用必须运行在Windows操作系统下可以试试Shenandoah；如果你接手的是遗留系统，软硬件基础设施和JDK版本都比较落后，那就根据内存规模衡量一下，对于大概4GB到6GB以下的堆内存，CMS一般能处理得比较好，而对于更大的堆内存，可重点考察一下G1。
...
> 一般来说，CMS是推荐的标配，一般生产环境采用ParNew+CMS组合。JDK默认使用Parallel Scavenge 是因为CMS不稳定可能会进行退化。
...
-XX:G1HeapWastePercent=5 // Sets the percentage of heap that you are willing to
waste.
-XX:G1MixedGCLiveThresholdPercent=85 // Sets the occupancy threshold for an old
region to be included in a mixed garbage collection cycle. (experimental)
-XX:G1HeapRegionSize=32M // Sets the size of a G1 region.
-XX:MaxGCPauseMillis=10000 // Sets a target value for desired maximum pause
time.
...
jdk8在Java 8 update 20推出了字符串重复数据删除优化，所以jdk版本大于此版本，可以尝试开启G1的`-XX:+UseStringDeduplication`

### 2.6.3 YGC和Full GC的发生频率

Eden空间不足时会执行young gc，Full GC会先触发young GC。而当老年代空间不足时，或者调用System.gc()时就会触发full gc。

当然也有特殊情况，那就是如果创建一个大对象，Eden区地方不够，就会放入老年代，如果老年代地方也不够就会触发Full GC。或者时发生Minor GC之前，判断老年代可用空间是否大于此次GC是所有新生代的对象容量，如果不满足则Full GC（也就是2.7.5的空间分配担保原则）。

## 2.7 内存分配与回收策略

Java的自动内存管理技术，其实本质上就是解决两个问题，一是自动给对象分配内存，二是自动回收给对象分配的内存。整个第二章已经介绍了对象是如何回收内存的，因此这里总结一下对象的内存分配。

对象的内存分配理论上都是在堆上分配，在经典分代的理论下，新生对象通常会分配在新生代中，少数情况下比如对象的大小超过阈值的话也会分配到老年代。Java虚拟机规范也没有规定新对象的创建和存储细节。这里以HotSpot虚拟机为例学习了解内存分配和回收策略。

### 2.7.1 对象优先在Eden分配

**首先，大多数情况下，对象首先在Eden区中进行分配，当Eden区没有足够的空间时，虚拟机就会发起一次Minor GC。**  

```java
private static final int _1MB = 1024 * 1024;
//VM: -verbose:gc -Xms20M -Xmx20M -Xmn10M -XX:+PrintGCDetails -XX:SurvivorRatio=8
//Xms 堆最小值 Xmx 堆最大值 Xmn新生代大小 SurvivorRatio新生代中eden和s0/s1的比例
//-verbose:gc 在控制台输出GC情况
public static void main(String[] args) {
    ...
    bytes4 = new byte[4 * _1MB];//出现一次MinorGC
}
```

运行结果：

![image-20221119195556621](https://mypic-12138.oss-cn-beijing.aliyuncs.com/blog/picgo/image-20221119195556621.png)

我们从上面可以清晰地看到，我们通过JVM参数将Java堆限制为20M，其中新生代为10M，Eden区为8M。当我们执行到上面代码最后一个byts4初始化时，Eden已经用了6M了，因此这时会发生一次MinorGC。从结果中也能看到这次现场，但是123都是存活的，因此几乎没有可回收的对象，且垃圾收集期间发现已有的三个对象无法放入S区，所以只能通过分配担保机制转移到老年代，因为三个2M的对象无法放入S区。这次GC后老年代用了6M（被对象123占用），Eden占4M（被刚分配的对象4占用）。

### 2.7.2 大对象直接进入老年代

大对象指的就是需要大量连续内存空间的对象，最典型的就是很长的字符串，或者元素数量很大的数组。再分配空间时，它容易导致内存明明还有不少空间但是却提前触发内存收集，以获取足够的连续空间安置它们，而当复制对象时，大对象意味着高额的内存复制开销。Hotspot虚拟机虚拟机提供了`-XX:PretenureSizeThreshold=byte`参数，指定当对象大于多少字节直接分配在老年代。

> 注意：-XX:PretenureSizeThreshold参数只对Serial和ParNew有效，jdk8默认的收集器是Parallel+Parallel Old，因此我做此实验时加了-XX:+UseParNewGC参数

```java
private static final int _1MB = 1024 * 1024;
//VM: -verbose:gc -Xms20M -Xmx20M -Xmn10M -XX:+PrintGCDetails -XX:SurvivorRatio=8
//-XX:PretenureSizeThreshold=3145728 这里3145728=3M
public static void main(String[] args) {
    //System.gc();
...
![image-20221119203608034](https://mypic-12138.oss-cn-beijing.aliyuncs.com/blog/picgo/image-20221119203608034.png)

从结果中也能看出来，我们6M的对象直接进入到了老年代，因为我们-XX:PretenureSizeThreshold参数设置为了3M。（PS：上图中的Eden的4M是JVM自身的数据）

### 2.7.3 长期存活的对象进入老年代

HotSpot虚拟机多数收集器都使用了分代收集管理堆内存，虚拟机给每个对象都定义了一个age年龄，存储在对象头。**对象通常在Eden产生，如果经历第一次Young GC，且能被Survivor容纳的话，该对象就进入S区，且年龄为1，在S1区每熬过一次Young GC年龄就加一岁，当年龄到一定程度（默认15），就会进入老年代。**这个阈值可以通过`-XX:MaxTenuringThreshold`设置。

> **注意Survivor空间中相同年龄所有对象大小的总和（meta-class大小）大于Survivor空间的一半会导致的提前进入老年区**。也就是2.7.4的内容。在原书深入理解Java虚拟机中之所以没有发生这种情况，是因为原书的运行环境中，新生代第一次回收后剩下的空间小于 1024K/2大小
>
> 我在jdk8版本下无法实现书中的例子，因此这里将内存地址放大，就可以复现书中的例子了。同时要使用-XX:+UseSerialGC，因为-XX:MaxTenuringThreshold对jdk8默认的收集器是Parallel无效。

```java
//VM: -XX:+UseSerialGC -verbose:gc -Xms160M -Xmx160M -Xmn80M -XX:+PrintGCDetails -XX:SurvivorRatio=8 -XX:MaxTenuringThreshold=15
public static void main(String[] args) {
    byte [] bytes1,bytes2,bytes3;
    bytes1 = new byte[_1MB*2];
    bytes2 = new byte[_1MB*32];
    bytes3 = new byte[_1MB*32];
    bytes3 = null;
    bytes3 = new byte[_1MB*32];
}
```

![image-20221119215830701](https://mypic-12138.oss-cn-beijing.aliyuncs.com/blog/picgo/image-20221119215830701.png)

将值调到15：

![image-20221119220012367](https://mypic-12138.oss-cn-beijing.aliyuncs.com/blog/picgo/image-20221119220012367.png)

这时可以看到当值为1时，对象在第二次GC后进入了老年代。而当值为15时，S区中还存在着bytes1对象。

### 2.7.4 动态对象年龄判断

为了更好的适应内存情况，HotSpot并不是永远要求对象的年龄必须达到阈值才能晋升老年代。如果在S区空间中所有相同年龄所有对象大小总和大于S区的一半，可以直接进入老年代，无需等到阈值的年龄。

这里不做演示，在jdk8中（我的具体版本是jdk1.8.0_341）版本中只要使用《深入理解Java虚拟机第三版》中的例子，即可复现。在上一节也注释了这一情况。
...
在发生Young GC前，虚拟机必须先检查老年代最大可用连续空间是否大于新生代所有对象总空间，如果大，那么Young GC是安全的。如果不成立此条件，虚拟机就会检查`-XX:HandlePromotionFailure`参数是否允许担保失败，如果允许则检查老年代最大可用连续空间是否大于历次晋升的平均大小，如果大于则尝试一次尽管有风险。如果不允许担保失败则进行FullGC。这里例子直接使用书中的例子（因为需要JDK6环境）

![image-20221119221337649](https://mypic-12138.oss-cn-beijing.aliyuncs.com/blog/picgo/image-20221119221337649.png)

![image-20221119221442922](https://mypic-12138.oss-cn-beijing.aliyuncs.com/blog/picgo/image-20221119221442922.png)
...
JDK中很多小工具都参考了UNIX的命名方式，比如`jps`就是与`ps`类似。`jsp`可以列出正在运行的虚拟机进程，并显示虚拟机的执行主类和本地虚拟机唯一ID（LVMID）。使用方式如下：
...
![image-20230328131615356](https://mypic-12138.oss-cn-beijing.aliyuncs.com/blog/picgo/image-20230328131615356.png)

这个命令对于本地虚拟机进程来说，LVMID与操作系统的进程ID是一致的。
...
用于监视虚拟机各种运行状态信息的命令行工具，可以显示本地或者远程虚拟机进程中的类加载、内存、垃圾收集、即时编译等运行时数据，在没有 GUI图形界面、只提供了纯文本控制台环境的服务器上，它将是运行期定位虚拟机性能问题的常用工具。

~~~sh
jstat [ option vmid [interval[s|ms] [count]]]
~~~

> 对于命令格式中的 VMID 与LVMID 需要特别说明一下:如果是本地虚拟机进程，VMID与LVMID 是一致的;如果是远程虚拟机进程，那 VMID 的格式应当是:
> `[protocol:][//]lvmid[@hostname[:port]/servername]`

参数interval和count代表查询间隔和次数，如果省略这两个参数则只查一次。假设每250ms查一次进程2784的垃圾收集情况，一共查20次，应该是`jstat -gc 2764 250 20`
...
| -gc               | 监视Java堆状况，包括两个Eden区、2个S区、老年代、永久代的容量，已用空间，垃圾收集时间合计等信息 |
| -gccapacity       | 监视内容与-gc 基本相同，但输出主要关注 Java 堆各个区域使用到的最大最小空间 |
| -gcutil           | 监视内容与-gc 基本相同，但输出主要关注已使用空间占总空间的百分比 |
| -gccause          | 监视内容与-gcutil 基本相同，但是会额外输出上次垃圾收集的原因 |
| -gcnew            | 监视新生代垃圾收集状况                                       |
| -gcnewcapacity    | 监视内容与-gcnew 基本相同，输出主要关注使用到的最大最小空间  |
| -gcold            | 监视老年代垃圾收集状况                                       |
| -gcoldcapacity    | 监视内容与-gcold 基本相同，输出主要关注使用到的最大最小空间  |
...
![image-20230328165013363](https://mypic-12138.oss-cn-beijing.aliyuncs.com/blog/picgo/image-20230328165013363.png)

### 3.1.3 jinfo

> Java配置信息工具

jinfo的作用是实时查看和调整虚拟机的各项参数，使用jps命令的-v参数可以查看虚拟机启动时显式指定的参数列表，但如果想知道未被显式指定的参数的系统默认值，除了去找资料外，就只能使用 jinfo 的 -flag 选项进行查询了 (如果只限于JDK6或以上版本的话，使用java` -XX:+PrintFlagsFinal`查看参数默认值也是一个很好的选择)。jinfo 还可以使用-sysprops 选项把虚拟机进程的 `System.getProperties()`的内容打印出来。这个命令在JDK 5 时期已经随着 Linux 版的JDK 发布，当时只提供了信息查询的功能，JDK 6之后，jinfo 在 Widows 和 Linux 平台都有提供，并且加入了在运行期修改部分参数值的能力(可以使用`-flag [+|-]name` 或者` -flag name=value` 在运行期修改一部分运行期可写的虚拟机参数值)。在JDK 6中，jinfo 对于 Windows 平台功能仍然有较大限制，只提供了最基本的 -flag 选项.
...
jmap命令用于生成堆存储快照，如果不使用jmap命令，要想获取 Java 堆转储快照也还有一些比较“暴力”的手段:警如`-XX:+HeapDumpOnOutOfMemoryError `参数，可以让虚拟机在内存溢出异常出现之后自动生成堆转储快照文件。可以使用`[Ctrl]+[Break]`键让虚拟机生成堆转储快照文件，又或者在 Linux 系统下通过 `Kill -3`命令发送进程退出信号“恐吓”一下虚拟机，也能顺利拿到堆转储快照。

jmap 的作用并不仅仅是为了获取堆转储快照，它还可以查询 finalize 执行队列、Java堆和方法区的详细信息，如空间使用率、当前用的是哪种收集器等。
和jinfo 命令一样，jmap 有部分功能在 Windows 平台下是受限的，除了生成堆转储快照的-dump 选项和用于查看每个类的实例、空间占用统计的 -histo 选项在所有操作系统中都可以使用之外，其余选项都只能在Linux/Solaris 中使用。
...
常用的option参数如下（图片来自深入理解JVM虚拟机第三版）：

| 选项           | 作用                                                         |
| -------------- | ------------------------------------------------------------ |
| -dump          | 生成Java堆转储快照，格式为`jmap -dump:live,format=b,file=heap.bin`，其中live子参数说明是否只dump出存活的对象。 |
...
JDK 提供jhat(JVM Heap Analysis Tool)命令与jmap 搭配使用，来分析jmap 生成的堆转储快照。ihat 内置了一个微型的 HTTP/Web 服务器，生成堆转储快照的分析结果后可以在浏览器中查看。不过实事求是地说，在实际工作中，除非手上真的没有别的工具可用，否则多数人是不会直接使用 jhat 命令来分析堆转储快照文件的，主要原因有两个方面。一是一般不会在部署应用程序的服务器上直接分析堆转储快照，即使可以这样做，也会尽量将堆转储快照文件复制到其他机器上进行分析，因为分析工作是一个耗时而且极为耗费硬件资源的过程，既然都要在其他机器上进行，就没有必要再受命令行工具的限制了。另外一个原因是 jhat 的分析功能相对来说比较简陋，可以使用VisualVM等工具代替。

使用示例：

![image-20230328172003030](https://mypic-12138.oss-cn-beijing.aliyuncs.com/blog/picgo/image-20230328172003030.png)

之后在浏览器输入`http://ip:7000/`即可访问并查看。

### 3.1.6 jstack

> Java堆栈跟踪工具

jstack (Stack Trace for Java)命令用于生成虚拟机当前时刻的线程快照(一般称为threaddump或者javacore 文件)。线程快照就是当前虚拟机内每一条线程正在执行的方法堆栈的集合，生成线程快照的目的通常是定位线程出现长时间停顿的原因，如线程间死锁死循环、请求外部资源导致的长时间挂起等，都是导致线程长时间停顿的常见原因。线程出现停顿时通过 jstack 来查看各个线程的调用堆栈，就可以获知没有响应的线程到底在后台做些什么事情，或者等待着什么资源。
...
![image-20230328172448687](https://mypic-12138.oss-cn-beijing.aliyuncs.com/blog/picgo/image-20230328172448687.png)

### 3.1.7 总结

上述一共简单介绍学习了6个基础工具，实际上JDK还提供了很多的其他的基础和高级工具，这里简单列一些，如下图（图片来自深入理解Java虚拟机第三版）：

![image-20230328172625498](https://mypic-12138.oss-cn-beijing.aliyuncs.com/blog/picgo/image-20230328172625498.png)

![image-20230328172718689](https://mypic-12138.oss-cn-beijing.aliyuncs.com/blog/picgo/image-20230328172718689.png)
