...
### 1.5 NIO文件传输CPU复制源码分析

下面我们结合NIO源码来看一下

当我们向对端发送数据时，需要调用write方法，将ByteBuffer传入，如下图：

![image-20221122094749222](https://mypic-12138.oss-cn-beijing.aliyuncs.com/blog/picgo/image-20221122094749222.png)

下面我们跟进write方法（源码来自openjdk）：
...
这里我们主要关注`IOUtil.write(fd, buf, -1, nd);`方法：
...
static int write(FileDescriptor fd, ByteBuffer src, long position,
    ...
    throws IOException
{
    //判断是否是直接内存，是直接内存的话调用writeFromNativeBuffer方法
    ...
    //如果不是直接内存就创建一个临时的DirectBuffer
    ByteBuffer bb = Util.getTemporaryDirectBuffer(rem);
    try {
        //CPU复制将ByteBuffer数据复制到临时直接内存
        bb.put(src);
        bb.flip();
        // Do not update src until we see how many bytes were written
        src.position(pos);
		调用writeFromNativeBuffer方法
...
从上面我们可以看到，当我们使用Java的IO时，如果不使用直接内存，那么需要进行一次CPU复制，将数据从JVM堆内存复制到堆外内存。

读也是同理，我们看一下read的openjdk源码，首先我们从SocketChannel#read(java.nio.ByteBuffer)进入
...
public int read(ByteBuffer buf) throws IOException {
    ...
    synchronized (readLock) {
        ...
        try {
            ...
            //这里也是调用IOUtil.read方法读取ByteBuffer
            for (;;) {
...
mmap可以将文件或其他对象映射到内存，它本身提供了不同于一般的普通文件的访问方式，让进程可以像读写内存一样对普通文件进行操作。下面我们来学习一下Java中如何使用mmap。

首先我们复习一下Java的文件IO，Java一般来说有三种文件IO：
...
- FileChannel文件通道
  ...
  ~~~java
  FileChannel fileChannel = new RandomAccessFile(new File("D:\\test.txt"), "rw").getChannel();
  //或者通过文件流获取
...
- MMAP内存映射
  ...
  mmap比较特殊，它是通过FileChannel的map方法衍生出来的一种特殊的读写文件的方式，称之为内存映射。

  ~~~java
  MappedByteBuffer mapBuffer = fileChannel.map(FileChannel.MapMode.READ_WRITE, 0, fileChannel.size());
  ~~~

- 

我们先来看一下MappedByteBuffer的继承关系：

![image-20221122132426351](https://mypic-12138.oss-cn-beijing.aliyuncs.com/blog/picgo/image-20221122132426351.png)

我们可以看到MappedByteBuffer继承了Buffer，内存维护了一个逻辑地址address属性。

然后我们看一下创建MappedByteBuffer的fileChannel.map方法，FileChannel是文件通道类，它提供了map方法用来将文件映射到虚拟内存，通常情况下可以映射整个文件，最大可以映射2G以内的文件。

> 在源码注解中可以看到最大可以映射2G即Integer.MAX_VALUE即2^31
>
...
- 文件映射类型，定义在FileChannel.MapMode中，共三种：
  - READ_ONLY，只读，如果试图修改缓冲区则会抛出异常
  - READ_WRITE，读写，对缓冲区的修改最终会写入到文件，对映射到同一文件的其他进程不一定可见。
  - PRIVATE，私用，可读可写，但是不会写入文件，只修改buffer，这种可以成为copy on write(注意这里有问题，我在测试时发现会将缓冲区大小的null刷入文件，暂时未找到原因)
- position，文件映射的起始位置
- size，映射缓冲区长度
...
public static void main(String[] args) {
    //字节数
    int length = 1024;
    try(RandomAccessFile randomAccessFile = new RandomAccessFile(new File("D:\\001\\001.txt"),"rw")) {
...
> 这里有问题，我在测试时发现会将缓冲区大小的null刷入文件，暂时未找到原因。我使用了jdk8和17，在windwos和linux中都进行了尝试，都是一样的结果，还没找到原因在哪里。
...
共享内存是UNIX中IPC机制的关键，共享内存对于数据共享、系统快速查询、动态配置、减少资源等都有优势。对于UNIX系统来说，共享内存分为普通共享内存和文件映射共享内存，但对于windows实际上只有文件映射共享内存一种方式，所以Java应用也只是创建映像文件共享内存。

当然对于Java来说基本上没有共享内存这个概念，只是在某些应用或场景中有用。共享内存有如下特点：

- 可以被多个进行访问
- 读写操作的进行执行读写操作时，其他进程不能写
- 多个进程可以交替的对某一共享内存执行写操作
- 一个进程执行写操作后，不影响其他进程的访问，同时其他进程对更新后的内存具有可见性

在Java中共享内存通过MappedByteBuffer实现共享内存，高缓冲区其实是磁盘文件的内存映射，而共享内存和文件建立联系的是FileChannel类。这里要注意写入时我们要保证共享内存的互斥，这里可以通过分布式锁实现，如果实在同一机器上，那么可以通过文件锁进行写操作互斥实现。下面是一个文件锁的共享内存的示例：

先定义一个共享内存类：

```java
package com.learn.shareMem;
...
public class ShareMemory {
    ...
    RandomAccessFile file;//文件大小
    FileChannel fileChannel;//文件通道
    MappedByteBuffer map;//mmap内存映射
    int fileSize = 1024;//默认大小

    public ShareMemory(int fileSize) {
        //初始化
        try {
            this.fileSize = fileSize;
            this.file = new RandomAccessFile("D:\\001\\001.txt", "rw");
            this.fileChannel = file.getChannel();
            map = this.fileChannel.map(FileChannel.MapMode.READ_WRITE, 0, fileSize);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
	
    public synchronized int write(int pos, int len, byte[] bytes) {
        if (pos >= fileSize || pos + len >= fileSize) {
            return 0;
        }
    ...
    public synchronized int read(int pos, int len, byte[] bytes) {
        if (pos >= fileSize) {
            return 0;
        }
        FileLock fileLock = null;
...
public class TestWrite {
    public static void main(String[] args) {
        ShareMemory memory = new ShareMemory(1024);
        ...
        service.scheduleAtFixedRate(new Runnable() {
            ...
            public void run() {
                index++;
                String str = "测试文本" + index;
                byte[] bytes = str.getBytes(StandardCharsets.UTF_8);
                memory.write(0, bytes.length, bytes);
                System.out.println("完成写入：" + new String(bytes, StandardCharsets.UTF_8));
            }
...
public class TestRead {
    public static void main(String[] args) {
        ...
        service.scheduleAtFixedRate(new Runnable() {
            @Override
            public void run() {
                String str = "测试文本" + 100000;
                int length = str.getBytes(StandardCharsets.UTF_8).length;
                byte [] bytes = new byte[length];
                memory.read(0, length, bytes);
                System.out.println("完成读取：" + new String(bytes, StandardCharsets.UTF_8));
            }
        }, 1, 1, TimeUnit.SECONDS);
    }
}
...
下面我们通过openJDK源码分析一下mmap的核心原理，首先我们看一下获取文件Channel的方法：
...
我们可以看到这里加了隐式锁，保证了该通道的线程互斥。

然后我们看看MappedByteBuffer是如何创建的，这里使用的是openJdk的源码[FileChannelImpl.java](https://github.com/openjdk/jdk8/blob/master/jdk/src/share/classes/sun/nio/ch/FileChannelImpl.java)，由于原方法比较长，这里给出关键部分：

![image-20221123220620505](https://mypic-12138.oss-cn-beijing.aliyuncs.com/blog/picgo/image-20221123220620505.png)

我们可以跟进newMappedByteBuffer方法，源码如下：
...
我们可以看到，因为访问权限问题所以这里是通过DirectByteBuffer的构造器生成了DirectByteBuffer对象，然后将其返回。

以上就是MappedByteBuffer的创建过程的源码分析。当我们通过get方法获取数据时，其实跟DirectByteBuffer一样的。因为本质上就是使用的DirectByteBuffer对象。底层采用unsafe.getByte方法，通过（address + offset）获取指定内存的数据。
...
接下来我们看一下mmap的两种内存回收方式，首先是程序内存不够时的主动回收内存，在上面创建的过程中，如果内存不够就会调用System.gc尝试回收，然后休眠100ms后再次尝试进行映射，如果在失败则直接抛出OOM异常。其次是由于MappedByteBuffer其实是DirectByteBuffer构造器创建的对象，因此其回收逻辑与DirectByteBuffer是一样的，都是虚引用的回收，可见3.3。

### 4.4 mmap+write进行文件传输实例

> 用[NIO实例](https://github.com/Loserfromlazy/Code_Career/blob/master/Java%E9%AB%98%E5%B9%B6%E5%8F%91%E5%AD%A6%E4%B9%A0/Java%E9%AB%98%E5%B9%B6%E5%8F%91%E7%BC%96%E7%A8%8B%E7%AF%87%E4%B8%83NIO%E4%B8%8ENetty.md#1145-nio%E5%AE%9E%E4%BE%8B)改造的，用mmap零拷贝进行了改造

服务端代码：

```java
@Slf4j
public class MmapReceiverFileServer {
    public static void main(String[] args) throws Exception {
        MmapReceiverFileServer server = new MmapReceiverFileServer();
        server.start();
    }
    ...
    private Map<Channel, MmapReceiverFileServer.Session> clientMap = new HashMap<>();
        ...
        while (selector.select()>0){
            ...
            while (iterator.hasNext()){
                ...
                if (next.isAcceptable()){
                    ...
                    MmapReceiverFileServer.Session session = new MmapReceiverFileServer.Session();
    ...
    private void handleData(SelectionKey selectionKey) throws IOException {
    ...
    private void process(ByteBuffer buffer, Session session) throws IOException {
        while (buffer.hasRemaining()&&buffer.remaining() > 0) {
            switch (session.step){
                case 1:
                ...
                case 3:
                    ...
                    session.mappedByteBuffer = fileChannel.map(FileChannel.MapMode.READ_WRITE, 0, session.fileLength);
                    session.step =4;
                ...
                case 4:
                    ...
                    if (session.position < session.fileLength){
                        ByteBuffer sliceMap = session.mappedByteBuffer.slice();
                        sliceMap.position(session.position);
                        sliceMap.put(slice);
                        //手动清除缓冲区，因为clear方法只是重置指针而已
                        buffer.limit(buffer.position());
                        session.position+=remaining;
                    }
                    log.info("已接收字节数={},总接收={}",buffer.remaining(),session.position);
                    if (session.fileLength <= session.position){
                        log.info("完成文件传输");
...
```java
@Slf4j
public class MmapSendFileClient {

    public static void main(String[] args) throws Exception {
        MmapSendFileClient client = new MmapSendFileClient();
        client.sendFile();
    }
    ...
    public void sendFile() throws Exception {
        ...
        while (pos<file.length()){
            ...
            slice.position(pos);
            int sendByte = pos + 1024;
            if (sendByte>file.length()){
                sendByte = (int) file.length();
            }
            slice.limit(sendByte);
            pos+=slice.remaining();
